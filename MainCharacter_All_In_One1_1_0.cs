...
public class PlayerMove : MonoBehaviour
{
    ...
    const string SKILL1 = "Crimson_Skill1";
    ...
    void Update() {
        ...
        if(Input.GetKeyDown(KeyCode.K)) {
            if(isIdleState()) {
                isAttack=true;
                ChangeAnimation(SKILL1);
                Invoke("AttackComplete", 3);
            }
        }
    ...
    void OnCollisionEnter2D(Collision2D collision) {
        if(collision.gameObject.tag == "TrapDoor") {
            if(transform.position.y > collision.transform.position.y && isAirAttack == true)
                onDestroy("TrapDoor", collision.transform);
        }
    }

    void onDestroy(string DestroyType, Transform Obj) {
        switch(DestroyType) {
            case "TrapDoor" :
